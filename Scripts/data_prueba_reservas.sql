...
Slots AS (
    SELECT
        DATEADD(HOUR, Ofs, @HorarioInicio) AS SlotDT,
        CAST(DATEADD(HOUR, Ofs, @HorarioInicio) AS TIME) AS Hora
    FROM Horas
),
Ocupacion AS (
    SELECT
        s.Hora,
        -- Ocupación cap.1: reservas con CantidadCarga >= 1
        (SELECT COUNT(*) FROM dbo.Reserva r
         WHERE r.FechaServicio   = @Fecha
           AND r.EstadoOperacion <> 'CANCELADO'
           AND r.CantidadCarga   >= 1
        ...
        -- Ocupación cap.2: reservas con CantidadCarga >= 2
        (SELECT COUNT(*) FROM dbo.Reserva r
         WHERE r.FechaServicio   = @Fecha
           AND r.EstadoOperacion <> 'CANCELADO'
           AND r.CantidadCarga   >= 2
           AND s.SlotDT >= CAST(r.FechaServicio AS DATETIME) + CAST(r.HoraInicio AS DATETIME)
           AND s.SlotDT <  CAST(r.FechaServicio AS DATETIME) + CAST(r.HoraFin AS DATETIME)
        ) +
        (SELECT COUNT(*) FROM dbo.TimerReserva tr
         WHERE tr.FechaServicio  = @Fecha
           AND tr.CantidadCarga  >= 2
           AND DATEADD(MINUTE, tr.TimerExpiracion, tr.FechaCreacion) > GETDATE()
           AND s.SlotDT >= CAST(tr.FechaServicio AS DATETIME) + CAST(tr.HoraInicio AS DATETIME)
           AND s.SlotDT <  CAST(tr.FechaServicio AS DATETIME) + CAST(tr.HoraFin AS DATETIME)
        ) AS OcupadoCap2,

        -- Ocupación cap.3: reservas con CantidadCarga >= 3
        (SELECT COUNT(*) FROM dbo.Reserva r
         WHERE r.FechaServicio   = @Fecha
           AND r.EstadoOperacion <> 'CANCELADO'
           AND r.CantidadCarga   >= 3
           AND s.SlotDT >= CAST(r.FechaServicio AS DATETIME) + CAST(r.HoraInicio AS DATETIME)
           AND s.SlotDT <  CAST(r.FechaServicio AS DATETIME) + CAST(r.HoraFin AS DATETIME)
        ) +
        (SELECT COUNT(*) FROM dbo.TimerReserva tr
         WHERE tr.FechaServicio  = @Fecha
           AND tr.CantidadCarga  >= 3
           AND DATEADD(MINUTE, tr.TimerExpiracion, tr.FechaCreacion) > GETDATE()
           AND s.SlotDT >= CAST(tr.FechaServicio AS DATETIME) + CAST(tr.HoraInicio AS DATETIME)
           AND s.SlotDT <  CAST(tr.FechaServicio AS DATETIME) + CAST(tr.HoraFin AS DATETIME)
        ) AS OcupadoCap3
...
SELECT
    ...
    @GruasCap1                                          As CantGruasCap1,
    @GruasCap2                                          As CantGruasCap2,
    @GruasCap3                                          As CantGruasCap3,
    OcupadoCap1,
    OcupadoCap2,
    OcupadoCap3,
    @GruasCap1 - OcupadoCap1                            AS LibresCap1,
    GREATEST(0, @GruasCap2 - OcupadoCap2)               AS LibresCap2,
    GREATEST(0, @GruasCap3 - OcupadoCap3)               AS LibresCap3
    /*
    CASE
        WHEN OcupadoCap1 >= @GruasCap1 THEN 'LLENO'
        WHEN OcupadoCap1 >= @GruasCap1 * 0.80 THEN 'CRÍTICO  (≥80%)'
        WHEN OcupadoCap1 >= @GruasCap1 * 0.50 THEN 'MODERADO (≥50%)'
        ELSE 'DISPONIBLE'
    END AS EstadoSlot,
    CAST(CAST(100.0 * OcupadoCap1 / NULLIF(@GruasCap1, 0) AS DECIMAL(5,1)) AS VARCHAR(8)) + '%' AS OcupacionPct*/
FROM Ocupacion
ORDER BY Hora
